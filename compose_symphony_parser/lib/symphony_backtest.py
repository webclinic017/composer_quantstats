...
def date_to_epoch_days(day: datetime.date) -> int:
    return int(
        datetime.datetime.combine(
            day, datetime.time(0, 0), tzinfo=UTC_TIMEZONE
        ).timestamp()
        / 60
        / 60
        / 24
...
def get_composer_backtest_results(
    symphony_id: str,
    start_date: datetime.date,
    end_date: typing.Union[datetime.date, None] = None,
) -> dict:
    ...
    while tries_remaining:
        try:
            response = requests.post(
                "https://backtest.composer.trade/v2/backtest", json=payload
            )
...
def extract_allocations_from_composer_backtest_result(
    backtest_result: dict,
) -> pd.DataFrame:
    composer_allocations = (
        pd.DataFrame(backtest_result[":tdvm-weights"]).fillna(0).round(4)
    )
...
def extract_returns_from_composer_backtest_result(
    backtest_result: dict, symphony_id: str
) -> pd.Series:
    ...
    returns = (
        pd.Series(
            [row[1] for row in returns_by_day], index=[row[0] for row in returns_by_day]
        )
        .pct_change()
        .dropna()
    )
